...
// Every 500k blocks (roughly 347 days) for 5 iterations the reward will decline by 0.1
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{

    int64_t nSubsidy = STATIC_POS_REWARD;

    if (nBestHeight+1 >= REWARD_FORK_BLOCK) {
      if (nBestHeight+1 >= 500000) {
        nSubsidy = 0.90 * COIN;
      } else if (nBestHeight+1 >= 1000000) {
        nSubsidy = 0.80 * COIN;
      } else if (nBestHeight+1 >= 1500000) {
        nSubsidy = 0.70 * COIN;
    ...
    if (!fIsInitialDownload)
    {
        if(MasternodePayments)
        {
		    ...
		    masternodePaymentAmount = 0.5 * COIN;
    		    ...
    		    if (nBestHeight+1 >= REWARD_FORK_BLOCK) {
      		      if (nBestHeight+1 >= 500000) {
        		masternodePaymentAmount = 0.45 * COIN;
      		      } else if (nBestHeight+1 >= 1000000) {
        		masternodePaymentAmount = 0.40 * COIN;
      		      } else if (nBestHeight+1 >= 1500000) {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (pfrom->nVersion < MIN_PEER_PROTO_VERSION && nBestHeight+1 >= REWARD_FORK_BLOCK)
        {
